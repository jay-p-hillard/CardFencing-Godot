...
func next_attacker(attacker_before: int, result: ExchangeResult, attacker_hand_empty: bool = false) -> int:
	...
	#Special Saber rule, if attack blocked, attacker can keep attacking if hand not empty
	if result.outcome == Outcome.Type.BLOCK && not attacker_hand_empty:
		return attacker_before
...
func should_refill_hands(result: ExchangeResult) -> bool:
	return result.outcome != Outcome.Type.BLOCK
